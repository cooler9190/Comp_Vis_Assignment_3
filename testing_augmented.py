...
def loading_path(name):
    if name == "normal": return "variant_pretrained_results_and_weights/45_percent_dropout_probability/history_variant_pretrained.pth"
    return "variant_pretrained_augmented_results_and_weights/45_percent_dropout_probability/history_variant_pretrained_augmented.pth"

def model_name(name):
    if name == "normal": return "Pre-trained model (on CIFAR100)"
    return "Augmented Pre-trained model (on CIFAR100)"

def matrix_name(name):
    if name == "normal": return "pretrained.png"
    return "augmented.png"

def test_model(name):
    model = PretrainedModel() # Instantiate the model.
    path = loading_path(name) # Get params file path.
    ...
    plt.title(f"Confusion Matrix for {model_name(name)}")
    ...
    fig_path = os.path.join(matrices_dir, matrix_name(name)) # Image name and location.
    plt.savefig(fig_path)  # Saves the image
    plt.show()

test_model("normal")
test_model("augmented")
